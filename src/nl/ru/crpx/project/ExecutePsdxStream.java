...
/* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- */
public class ExecutePsdxStream extends Execute {
  protected General objGen;   // Local copy of the General object
  ...
  /* ---------------------------------------------------------------------------
     ...
     20/apr/2015   ERK Created
     --------------------------------------------------------------------------- */
  public ExecutePsdxStream(CorpusResearchProject crpThis, General oGen) {
    // Do the initialisations for all Execute() classes
    super(crpThis, oGen);
    this.objGen = oGen;      // Set the right pointer to the General object
  }

  /* ---------------------------------------------------------------------------
     Name:    ExecuteQueriesPsdxStream
     Goal:    Execute the queries in the given order for Xquery type processing
              We go file by file, <forest> by <forest>, and execute the queries
     ...
     --------------------------------------------------------------------------- */
  public static boolean ExecuteQueriesPsdxStream() {
    try {
      ...
      // Warn user
      DoError("modMain/ExecuteQueriesXqueryStream error: " + ex.getMessage() + "\r\n");
      // Return failure
      return false;
    }
  }
